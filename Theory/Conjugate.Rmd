---
title: "An example of conjugate distributions"
...
# Demonstration
  ...
  \begin{eqnarray}
\left[\theta \right|s]&=& \frac{\frac{1}{B(\alpha,\beta)}\theta^{s}(1-\theta)^{n-s}\theta^{\alpha-1}(1-\theta)^{\beta-1}\mathbbm{1}_{\left[0,1\right]} }{\frac{1}{B(\alpha,\beta)}\int_0^1\theta^{s}(1-\theta)^{n-s}\theta^{\alpha-1}(1-\theta)^{\beta-1}\mathbbm{1}_{\left[0,1\right]} d\theta} \\
\left[\theta \right|s]&=& \frac{\theta^{s}(1-\theta)^{n-s}\theta^{\alpha-1}(1-\theta)^{\beta-1}\mathbbm{1}_{\left[0,1\right]} }{\int_0^1\theta^{s}(1-\theta)^{n-s}\theta^{\alpha-1}(1-\theta)^{\beta-1}\mathbbm{1}_{\left[0,1\right]} d\theta} \\
\left[\theta \right|s]&=& \frac{\theta^{s+\alpha-1}(1-\theta)^{n-s+\beta-1}\mathbbm{1}_{\left[0,1\right]} }{\int_0^1\theta^{s+\alpha-1}(1-\theta)^{n-s+\beta-1}\mathbbm{1}_{\left[0,1\right]}d\theta}
  \end{eqnarray}
...
# So...
  ...
  \begin{eqnarray}
  \left[\theta \right|s]&=& \frac{\theta^{\alpha+s-1}(1-\theta)^{n-s+\beta-1}\mathbbm{1}_{\left[0,1\right]} }{B(n+\alpha-s, s+\beta)}\\
  \left[\theta \right|s]&\sim&\mathcal{B}eta(\alpha+s,n-s+\beta)
  \end{eqnarray}
...
lines(seqx, dbeta(seqx, beta2, alpha2), type="l", col=myblue, lwd=2)
```
