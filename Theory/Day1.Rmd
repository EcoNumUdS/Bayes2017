...
\begin{exampleblock}{PRACTICE 1}
  \begin{itemize}
      \item<1-5> $P(\overline{A} \cap B) = f(P(A), P(B), P(A \cap B))$
      \item<2-5> $P(A \cup B \cup C)$
      \item<3-5> the duck hunter 1 bullet
      \item<4-5> the duck hunter 2 bullets / 2 ducks - 1 duck
      \item<5-5> bonus: how to simulate a dice with a coin?
  \end{itemize}    
\end{exampleblock}
...
points(0:10+.12, dbinom(0:10, 10, .25), col=myblue, type="h", lwd=3)
points(0:10-.12, dbinom(0:10, 10, .75), col=mygreen, type="h", lwd=3)
legend("top", legend = c("p=.25", "p=.5", "p=.75"), col = c(mygreen, mygrey, myblue) , lwd = 3, bty="n", ncol = 3, cex=1.4)
```
...
par(mypar)
plot(c(0.5,20.5), c(0,.6), xlab="Values (x)", ylab="Probability", type="n")
points(0:20, dpois(0:20, 1), col=myblue, type="h", lwd=3)
points(0:20+.12, dpois(0:20, 5), col=mygrey, type="h", lwd=3)
points(0:20-.12, dpois(0:20, 10), col=mygreen, type="h", lwd=3)
legend("top", legend = c("p=5", "p=10", "p=20"), col = c(mygreen, mygrey, myblue) , lwd = 3, bty="n", ncol = 3, cex=1.4)
```
...
# Infinite sets - where's the duck?


```{r whereisb, echo = FALSE, fig.width=7, fig.height=4.5}
par(xaxs="i", yaxs="i")
plot0()
circle(0, 0, col=myblue, border=NA)
rd <- runif(1)*.99
...
```{r whereisc, echo = FALSE, fig.width=7, fig.height=4.5}
par(xaxs="i", yaxs="i")
plot0()
circle(0, 0, col=myblue, border=NA)
rd <- runif(20)*.99
...
$f$ is a p.d.f iif:
  ...
  - $f$ is defined...
    - on [a,b]  (a may be -$\infty$ / b may be +$\infty$)
    - positive  
    - regular
...
\pause
  ...
  - **probability mass function, p.m.f.**: random variables with a discrete support set (add countable infinite)
\pause

  - **density mass function, p.m.f.**: random variables with a discrete support set
...
Everthing holds true:

  - conditional p
  - $x_1 \cap x_2$: $f(x_1)f(x_2)$


**Cumulative distribution function**

$$F(x) = P(X<x) = \int_{-\infty}^x$$
...
lines(seqx, dnorm(seqx, 2), col=mygrey, lwd=4, main = "dnorm")
lines(seqx, dnorm(seqx, 1, 2), col=mygreen, lwd=4, main = "dnorm")
```

<!-- add the Cumulative funtioj -->



# Lognormal distribution *dlnorm*
...
- You've set up a meeting with three colleagues:
  ...
  3. right on time if the starting time were 1 hour eralier...

- How to model the time upon arrival?
- When will you start the meeting?
- Bonus: $X_1 \rightarrow \mathcal{N}(0,1)$ and $X_2 \rightarrow \mathcal{N}(0,1)$
...
Set of $B_i$ is a partition of B,

$$P(A|B) = \frac{P(B|A)P(A)}{\sum P(B_i)}$$

$$f(A|B) = \frac{f(B|A)f(A)}{\int f(b)db}$$



# Practice 4 (30 min)

- prevalence $\alpha$ (0.001)
- test to detect the disease error type 1 $\beta$ (.05)
- You take the test, it's positive, are you sick?
- Same question if the test is negative
...
the duck hunter is back (30 min)

- the precision decrease with distance $f(d) = exp(-2*d)$

- the distance distribution account for :
  - duck escape
  - the self is a lognormal (takes)

- he brought 10 bullets, what's the probability he will have a nice diner tonight


# the duck hunter... the truth about him

- table of success and failure with and without distance...
- update info !




# Let's take a step back (10 min)
...
- Realisation of a random variable $X$ distribution is $f(\theta)$

- and we try to say something about $\theta$ given $x_i$

- **Inference**

# Let's take a step back

- To do so we build **estimators** this could be tricky

- Normal ...

- Variance if we used the variace... we should use 1/n+1 (you can demonstrate this)

- Then we try to assess the goodness of our estimation : IC / tests

- Likelihood ... Bayesian misture and update information.. but there are links
